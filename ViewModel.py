...
# Initialize project and datasets' tables for images
try:
    conn = dbm.openConn()
    c = conn.cursor()
    c.execute("CREATE TABLE IF NOT EXISTS projects (p_url TEXT PRIMARY KEY, p_id INTEGER, p_name TEXT, p_hash TEXT, p_image BLOB, p_img_hash TEXT, p_updated INTEGER)")
    c.execute("CREATE TABLE IF NOT EXISTS datasets (ds_p_url TEXT, ds_url TEXT PRIMARY KEY, ds_id INTEGER, ds_name TEXT, ds_hash TEXT, ds_image BLOB, ds_img_hash TEXT)")
    dbm.closeConn(c)
    # print("Success when initializing tables for images")
except Exception:
    print("Something failed when initializing tables for images")